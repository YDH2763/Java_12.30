...
/*
학생 등록
 - 학생의 전공, 이름, 학년, 연락처, 주민번호를 입력하여 학생을 등록한다.
...
과목 등록/수정/삭제
 - 과목코드, 과목명, 이수 학점, 시간, 타입(전공필수, 전공선택, 교양필수, 교양선택, 교직필수 등....)
 - 과목코드는 분류코드 3자리, 순서 3자리로 구성된다.
 - 분류코드는 교직(TEA), 교양(CUL), 기본(MSC) 전공별로 할당된 코드를 사용한다.
 - 이수학점은 이수 시 인정되는 학점으로 0~4학점까지이다.
 - 시간은 1주일 강의 시간으로 0~4시간이다.
 - 같은 내용의 수업이라도 전공마다 각각 등록한다.
강의 등록/수정/삭제
 - 등록된 과목, 교수를 선택하고, 년도, 학기, 시간표, 강의장, 강의계획서, 분반, 최대 정원을 입력하여 등록한다.
 - 년도는 강의가 개설되는 연도 4자리이다.
 - 학기는 1, 2, 여름, 겨울 중 하나이다.
 - 시간표는 강의 시간표로 1A,1B 형태로 나타내며 A,BSMS 30분 간격이다.
