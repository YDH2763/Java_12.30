...
# 아이디 : abc123, 비밀번호 : abc123, 이메일 : abc123@db.com, 전화번호 :111-1234-5678
...
VALUE("abc123", "abc123", "abc123@db.com", "111-1234-5678");
 ...
 # 아이디 : admin123, 비밀번호 : admin123, 이메일 : admin123@db.com, 전화번호 :111-2222-3333
 
INSERT INTO MEMBER (ME_ID, ME_PW, ME_EMAIL, ME_NUMBER, ME_AUTHORITY)
VALUE("admin123", "admin123", "admin123@db.com", "111-2222-3333", "ADMIN");

# 카테고리를 등록할 때 필요한 쿼리
# 상의 :TOP, 하의 : PAN, 악세서리 : ACC, 모자 : CAP, 신발 : SHO
...
BEGIN
	...
	# PR_CODE가 _CODE로 시작하는 제품의 개수 + 1를 구함
    ...
    # 제품 코드는 _CODE와 위에서 구한 3자리 문자열을 합해서 구함
    SET _CODE = CONCAT(_CA_CODE, _COUNT_STR);
    # _CODE를 이용해서 PR CA NUM를 구함
    SET _CA_NUM = (SELECT CA_NUM FROM CATEGORY WHERE CA_CODE= _CA_CODE);
    
    # 위에서 구한 값들을 이용하여 INSERT문 작성
	INSERT INTO PRODUCT(PR_CODE, PR_TITLE, PR_CONTENT, PR_PRICE, PR_THUMB, PR_CA_NUM)
		   VALUES(_CODE, _TITLE, _CONTENT, _PRICE, _THUMB, _CA_NUM);
END //
DELIMITER ;
...
CALL INSERT_PRODUCT("ACC", "순금반지 14K", "돌반지", 400000, "/IMG/ACC_03.JPG");

CALL INSERT_PRODUCT("CAP", "빈티지 모자", "빈티지 모자입니다.", 13000, "/IMG/CAP_01.JPG");
CALL INSERT_PRODUCT("CAP", "위생 모자", "위생 모자입니다.", 15000, "/IMG/CAP_02.JPG");
CALL INSERT_PRODUCT("CAP", "선캡", "모자입니다.", 20000, "/IMG/CAP_03.JPG");

CALL INSERT_PRODUCT("SHO", "장화", "장화입니다.", 20000, "/IMG/SHO_01.JPG");
CALL INSERT_PRODUCT("SHO", "런닝화", "편한 런닝화", 90000, "/IMG/SHO_02.JPG");
CALL INSERT_PRODUCT("SHO", "구두", "스타일 좋은 구두", 160000, "/IMG/SHO_03.JPG");

# abc123회원이 TOP001 제품을 장바구니에 3개 담았을 때 쿼리
-- INSERT INTO CART VALUE(1,"abc123", "TOP001", 3);
INSERT CART(CT_ME_ID, CT_PR_CODE, CT_AMOUNT) VALUE("abc123", "TOP001", 3);
# abc123회원이 SHO001 제품을 장바구니에 1개 담았을 때 쿼리
INSERT CART(CT_ME_ID, CT_PR_CODE, CT_AMOUNT) VALUE("abc123", "SHO001", 1);

# abc123회원이 장바구니에 담긴 모든 제품을 구매할 때 쿼리
# 구매 테이블에 총 가격과 구매자를 추가하는 쿼리
INSERT INTO BUY(BU_ME_ID, BU_TOTAL_PRICE) VALUES("abc123","50000");
