...
SELECT * FROM PRODUCT JOIN CATEGORY ON CA_NUM = PR_CA_NUM WHERE CA_NAME='상의';
SELECT * FROM PRODUCT JOIN CATEGORY ON CA_NUM = PR_CA_NUM WHERE CA_CODE='TOP';
SELECT * FROM PRODUCT WHERE PR_CA_NUM =(SELECT CA_NUM FROM CATEGORY WHERE CA_NAME='상의');
...
# 사용자별 누적 구매액의 조회하는 쿼리
# - OUTER JOIN 사용 : 구매 내역이 없는 사용자도 조회하기 위해
# - GROUP BY, SUM : 사용자별 구매액을 계산하기 위해 

# 내가 한거 (오답)
-- SELECT MEMBER.*, IFNULL(SUM(BU_TOTAL_PRICE),0) AS 누적금액 FROM MEMBER  
-- JOIN BUY ON BU_ME_ID=ME_ID GROUP BY BU_TOTAL_PRICE;

# 정답
 SELECT ME_ID, IFNULL(SUM(BU_TOTAL_PRICE),0)AS 누적_구매량 FROM BUY
 RIGHT JOIN MEMBER ON ME_ID = BU_ME_ID
 GROUP BY ME_ID;

# "반"으로 제품을 검색했을 때 조회하는 쿼리
SELECT * FROM PRODUCT WHERE PR_TITLE LIKE CONCAT("%", "반", "%") 
OR PR_CONTENT LIKE CONCAT("%", "반", "%");
#내가 한거
-- SELECT * FROM PRODUCT WHERE PR_TITLE LIKE "반%";

# abc123 회원이 구매한 제품들을 구매확정을 했을 때 쿼리
UPDATE BUY SET BU_STATE ='확정',
	   BU_FINAL_DATE = NOW() WHERE BU_ME_ID ="abc123"; # 실제는 BU_NUM을 이용
