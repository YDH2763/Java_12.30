...
SELECT * FROM MOVIE WHERE MV_STATE="상영중";

# CGV강남을 선택하고, 미키17을 선택했을 때 상영 가능한 날짜를 조회하는 쿼리
...
# CGV강남 1관의 3월 11일, 16:20 에 상영하는 미키 17(3)의 좌석을 조회하는 쿼리
SELECT SE_NAME FROM SEAT
JOIN `SCHEDULE` ON SD_SC_NUM = SE_SC_NUM
WHERE SE_POS="Y" AND SD_NUM = 1;
...
WHERE SE_POS="Y" AND SD_NUM = 2 AND TL_SE_NUM = SE_NUM AND TI_STATE ="결재";

# CGV강남 4관의 3월 11일, 19:10 에 상영하는 미키 17의 예약 가능한 좌석을 조회하는 쿼리
# 예매 리스트와 JOIN 해서 예매가 안된 좌석을 조회
SELECT * FROM SEAT
...
LEFT JOIN MOVIE_GENRE ON GR_NAME = MG_GR_NAME GROUP BY GR_NAME;

# 현재 시간을 기준으로 한달 사이에 개봉한 영화를 조회하는 쿼리

SELECT * FROM MOVIE
WHERE
	MV_DATE BETWEEN DATE_SUB(NOW(), INTERVAL 1 MONTH) AND NOW() AND MV_STATE ="상영중";
...
LEFT JOIN TICKET ON TI_SD_NUM = SD_NUM 
WHERE TI_STATE = "결재" AND CONCAT(SD_DATE, " ", SD_TIME) >= NOW()
GROUP BY MV_NUM;
