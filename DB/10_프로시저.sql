...
프로시저에서 사용하는 문법
1. 변수 선언
 - 변수는 프로시저 시작 위치에 모아 놓아야 함. 중간에 선언할 수 없다.
DECLEAR 변수명 자료형 [DEFAULT 초기값];
2. 겁색 결과를 변수에 저장
...
7. 반복문 - CURSOR
 - 검색 결과를 반복문으로 활용할 때
DECLARE 변수명A BOOLEAN DEFAILT FALSE;
...
BEGIN
    ...
    DECLARE _CURSOR CURSOR FOR
		SELECT SC_ST_KEY, SC_SCORE, SJ_SEMESTER AS SC_GRADE, SJ_GRADE AS SC_SEMESTER
		FROM SCORE
	...
	CURSOR_LOOP : LOOP 
		...
		FETCH _CURSOR INTO _SC_ST_KEY, _SC_SCORE, _SC_GRADE, _SC_SEMESTER;
