...
영화 예매 사이트(CGV)
조회(무비 차트)
 - 사용자는 현재 상영 중이거나 상영 예정인 영화를 조회한다.
 - 영화는 예매 내역이 많은 순으로 조회한다.
조회(상영 시간) - 예매하기 전
 - 사용자는 영화, 극장, 날짜를 선택해서 상영시간을 확인한다.
 - 사용자는 현재 상영 중아거나 상영 예정인 영화를 선택한다.
 - 사용자는 현재 운영중인 극장을 선택한다.
 - 사용자는 상영시간에 등록된 날짜를 선택한다.
 - 사용자는 영화 극장 날짜를 선택하여 상영시간 및 관과 전체 좌석 수/예매 가능 가석 수를 확인한다.
조회(영화 상세 조회)
 - 사용자는 영화를 선택하여 상세 조회를 한다.
 - 상세 정보는 감독, 배우, 장르, 기본정보, 개봉일 , 영화 설명, 트래일러, 스틸컷이 있다.
 - 감독은 영화를 연출한 감독으로 등록된 감독 모두를 조회한다.
 - 배우는 영화에 참여한 배우로, 영화를 등록할 때 참여한 배우를 등록한 순으로 조회한다.
 - 장르는 영화 장르로, 등록된 장르 모드를 조회한다.
...
영화 관리
 ...
 - 영화 정보는 영화에 참여한 감독, 배우, 제목, 기본정보, 개봉일, 내용, 스틸컷, 트레일러, 영화 상세 정보를 입력하여 등록한다.
 - 감독/배우는 등록된 감독/배우 중에서 선택하여 입력하고, 여러명 입력이 가능하다.
 - 장르는 등록된 장르를 선택하며, 여러 장르 선택이 가능하다.
 - 기본 정보에서 연령은 등록된 연령중 하나를 선택한다.
...
감독/배우 관리
 ...
 - 감독/배우는 등록된 인물과 역활, 대표 사진을 선택하여 등록한다.
...
상영관 관리
 ...
 - 좌석 정보는 행은 영어 대문자로, 열은 숫자로 표현하여 해당 상영관의 모든 좌석수를 등록한다.
