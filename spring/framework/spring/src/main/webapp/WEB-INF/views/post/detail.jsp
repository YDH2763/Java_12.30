...
<body>
	...
	<div class="comment-container">
		<div class="comment-list">
			<!-- 
			<div class="comment-item form-control" style="min-height: auto; height: auto;">
		...
		<div class="comment-insert-box">
			<form class="input-group mb-3 insert-form" action="<c:url value="/comment/insert"/>" method="post">
		    	...
		    	<textarea rows="" cols="" class="form-control" name="co_content"></textarea>
		   		<button class="btn btn-outline-success">답글 등록</button>
		  	</form>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function drawCommentList(list){
			...
			if(list.length ==0){
				$(".comment-list").html(`<div class="text-center mb-3">등록된 게시글이 없습니다.</dic>`)
			}
			let str = '';
			for(comment of list){
				let btns='';
	...
	<script type="text/javascript">
		$(document).on("submit",".insert-form",function(e){
			...
			let co_comtent = $(this).find("[name=co_content]").val();
			
			let obj={
				co_po_num : co_po_num,
				co_content : co_comtent,
